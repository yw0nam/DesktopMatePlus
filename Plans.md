...
| Task | Content | DoD | Depends | Status |
|------|------|-----|---------|--------|
| MET-1 | FastAPI `/metrics` 엔드포인트 — request latency, error rate, active connections | `GET /metrics` → Prometheus 포맷 응답 | - | cc:TODO |
| MET-2 | `scripts/metrics.sh` — worktree별 메트릭 조회 + 임계값 초과 알림 | `scripts/metrics.sh --latency p99` 정상 출력 | MET-1 | cc:TODO |
