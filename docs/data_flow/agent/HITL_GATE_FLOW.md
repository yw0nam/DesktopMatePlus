...
| Category | 승인 필요 | 예시 | 동작 |
|----------|:---------:|------|------|
| `read_only` | ❌ | `read_file`, `list_directory`, `search_knowledge`, `read_note`, `search_memory`, `duckduckgo_search` | Bypass — handler 직접 실행 |
| `state_mutating` | ✅ | `write_file`, `add_memory`, `update_memory`, `delete_memory`, `update_user_profile` | Interrupt → 승인 대기 |
| `external` | ✅ | `delegate_task` | Interrupt → 승인 대기 |
| `dangerous` | ✅ | `terminal`, 모든 MCP 도구 (default), 미등록 도구 (fail-closed) | Interrupt → 승인 대기 |

> 카테고리 정의는 `_DEFAULT_CATEGORIES` (src/services/agent_service/middleware/hitl_middleware.py)가 단일 진실 소스. YAML `hitl_overrides`로 per-tool override.

분류 로직: `HitLMiddleware.get_category(tool_name)` — `_DEFAULT_CATEGORIES` + YAML overrides + MCP default/overrides가 startup에 병합된 category_map을 사용. 미등록 툴은 fail-closed로 `dangerous`.
...
Agent tool call (dangerous)
  → HitLMiddleware.awrap_tool_call()
    → interrupt({tool_name, tool_args, request_id, category})
...
`read_only` 카테고리는 middleware가 `interrupt()`를 호출하지 않고 `handler(request)`를 직접 await 한다. 프론트에 `hitl_request` 이벤트가 발생하지 않으며, 로그에는 다음과 같이 남는다:

```
DEBUG | HitL gate: 'search_memory' bypass (category=read_only)
```
...
| 파일 | 역할 |
|------|------|
| `src/services/agent_service/middleware/hitl_middleware.py` | `_DEFAULT_CATEGORIES` 단일 진실 소스, `get_category` / `requires_approval`, interrupt payload에 `category` 포함 |
...
2026-04-17: Phase 2 — 4-tier category 시스템 도입. `is_dangerous` → `get_category`, interrupt payload에 `category` 추가, `_DEFAULT_CATEGORIES` 단일 소스 + YAML `hitl_overrides`/`mcp_hitl_overrides`. Issue #42.
